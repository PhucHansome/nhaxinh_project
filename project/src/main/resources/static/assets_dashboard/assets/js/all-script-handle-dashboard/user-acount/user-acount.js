let page = {
    url: {
        getAllUser: App.BASE_URL_USER_ABC,
        getUserById: App.BASE_URL_USER_ABC + "/id/",
        PutBlock: App.BASE_URL_USER_ABC + "/block",
        PutActive: App.BASE_URL_USER_ABC + "/active"
    },
    element: {},
    loadData: {},
    commands: {},
    dialogs: {
        element: {},
        loadData: {},
        commands: {},
        close: {},
        alertDanger: {},
        inputError: {}
    }
}

let user = new User();
page.element.userName = $(".userName")
page.element.tbodyList = $("#listUser tbody")


page.loadData.getAllUser = () => {
    $.ajax({
        "method": "GET",
        "url": page.url.getAllUser
    }).done((data) => {
        page.element.tbodyList.html("")
        $.each(data, (i, item) => {
            user = item;
            if (user.username === page.element.userName.text()) {
                return;
            }
            if (user.status === "Block") {
                if (user.role.code === "USER") {
                let str = `
                     <tr>
                        <td class="text-center align-middle">${user.id}</td>
                        <td class="text-center align-middle"><img width="50px" height="50px"
                                                                  src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/59/User-avatar.svg/2048px-User-avatar.svg.png"
                                                                  alt=""></td>
                        <td class="text-center align-middle" style="font-weight: bold;">${user.username}</td>
                        <td class="text-center align-middle"><span      style="background: red;
                                                                            padding: 10px 10px 10px 10px;
                                                                            border-radius: 10px;
                                                                            font-weight: bold;
                                                                            color: white;">${user.status}</span> </td>
                        <td class="text-center align-middle">
                            <button data-id="${user.id}" class="btn btn-outline-success btn-btn-active"><i class="fa-solid fa-lock-open"></i></button>
                        </td>
                    </tr>
                                  `
                    page.element.tbodyList.append(str)
                }
                if (user.role.code === "ADMIN") {
                    let str = `
                         <tr>
                            <td class="text-center align-middle">${user.id}</td>
                            <td class="text-center align-middle"><img width="80px" height="80px"
                                                                              src="data:image/jpeg;base64,"
                                                                              alt=""></td>
                            <td class="text-center align-middle" style="font-weight: bold;">${user.username}</td>
                            <td class="text-center align-middle"><span style="background: red;
                                                                                padding: 10px 10px 10px 10px;
                                                                                border-radius: 10px;
                                                                                font-weight: bold;
                                                                                color: white;">${user.status}</span> </td>
                            <td class="text-center align-middle">
                                <button data-id="${user.id}" class="btn btn-outline-success btn-btn-active"><i class="fa-solid fa-lock-open"></i></button>
                            </td>
                        </tr>
                                    `
                    page.element.tbodyList.append(str)
                    pageAcount();

                }
            }
            if (user.status === "Active") {
                if (user.role.code === "USER") {
                    let str = `
                             <tr>
                                <td class="text-center align-middle">${user.id}</td>
                                <td class="text-center align-middle"><img width="50px" height="50px"
                                                                          src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/59/User-avatar.svg/2048px-User-avatar.svg.png"
                                                                          alt=""></td>
                                <td class="text-center align-middle" style="font-weight: bold;">${user.username}</td>
                                <td class="text-center align-middle"><span   style="background: #26d126;
                                                                                    padding: 10px 10px 10px 10px;
                                                                                    border-radius: 10px;
                                                                                    font-weight: bold;
                                                                                    color: white;">${user.status}</span> </td>
                                <td class="text-center align-middle">
                                    <button data-id="${user.id}" class="btn btn-outline-danger btn-btn-block"><i class="fa-sharp fa-solid fa-lock"></i></button>
                                </td>
                            </tr>
                                    `
                    page.element.tbodyList.append(str)
                    pageAcount();

                }
                if (user.role.code === "ADMIN") {
                    let str = `
                             <tr>
                                <td class="text-center align-middle">${user.id}</td>
                                <td class="text-center align-middle"><img width="80px" height="80px"
                                                                                  src="data:image/jpeg;base64,"
                                                                                  alt=""></td>
                                <td class="text-center align-middle" style="font-weight: bold;">${user.username}</td>
                                <td class="text-center align-middle"><span style="background: #26d126;
                                                                                    padding: 10px 10px 10px 10px;
                                                                                    border-radius: 10px;
                                                                                    font-weight: bold;
                                                                                    color: white;" >${user.status}</span> </td>
                                <td class="text-center align-middle">
                                    <button data-id="${user.id}" class="btn btn-outline-danger btn-btn-block"><i class="fa-sharp fa-solid fa-lock"></i></button>
                                </td>
                            </tr>
                                `
                    page.element.tbodyList.append(str)
                    pageAcount();

                }
            }
        })
        page.commands.clickBtnBlock();
        page.commands.clickBtnActive();
    }).fail((e) => {
        console.log(e);
        App.IziToast.showErrorAlert("Lỗi sever");
    })
}

page.commands.getUserById = (id) => {
    return $.ajax({
        "method": "GET",
        "url": page.url.getUserById + id
    }).done((data) => {
        user = data;
    }).fail((e) => {
        console.log(e)
    })
}

page.commands.clickBtnBlock = () => {
    $(".btn-btn-block").on("click", function () {
        let id = $(this).data("id");
        page.commands.getUserById(id).then(function () {
            Swal.fire({
                title: 'Bạn có muốn Block tài khoản này không?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Đúng, Tôi muốn đổi trạng thái!'
            }).then((resuilt) => {
                if (resuilt.isConfirmed) {
                    $.ajax({
                        headers: {
                            accept: "application/json",
                            "content-type": "application/json"
                        },
                        type: "PUT",
                        url: page.url.PutBlock,
                        data: JSON.stringify(user)
                    }).done((data) => {
                        App.SweetAlert.showSuccessAlert("Bạn đã khóa thành công")
                        page.loadData.getAllUser()
                        page.commands.clickBtnActive();
                    }).fail((E) => {
                        console.log(E);
                        App.IziToast.showErrorAlert("Block thất bại")
                    })
                }
            })
        })
    })
}

page.commands.clickBtnActive = () => {
    $(".btn-btn-active").on("click", function () {
        let id = $(this).data("id");
        page.commands.getUserById(id).then(function () {
            Swal.fire({
                title: 'Bạn có muốn Active tài khoản này không?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Đúng, Tôi muốn đổi trạng thái!'
            }).then((resuilt) => {
                if (resuilt.isConfirmed) {
                    $.ajax({
                        headers: {
                            accept: "application/json",
                            "content-type": "application/json"
                        },
                        type: "PUT",
                        url: page.url.PutActive,
                        data: JSON.stringify(user)
                    }).done((data) => {
                        App.SweetAlert.showSuccessAlert("Bạn đã mở khóa thành công")
                        page.loadData.getAllUser()
                        page.commands.clickBtnBlock()
                    }).fail((E) => {
                        console.log(E);
                        App.IziToast.showErrorAlert("Mở khoá thất bại")
                    })
                }
            })
        })
    })
}
$("#searchAcount").on("keyup", function() {
    var value = $(this).val().toLowerCase();
    $("tbody tr").filter(function() {
        $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
    });
    pageAcount();

});
var table = '#listUser';

function pageAcount(){
    $('#maxRows').on('change',function (){
        $('.pagination').html('')
        var trnum = 0;
        var marRows = parseInt($(this).val())
        var totalRows = $(table+' tbody tr').length
        $(table+ 'tr:gt(0)').each(function (){
            trnum ++
            if (trnum > marRows){
                $(this).hide()
            }
            if (trnum <= marRows){
                $(this).show()
            }
        })
        if (totalRows > marRows){
            var pagenum = Math.ceil(totalRows/marRows)
            for (var i=1;i<=pagenum;){
                $('.pagination').append('<li class="page-item" data-page=" ' + i + '" ><a class="page-link"\<span>' + i++ +' <span class="sr-only">(current)</span> </span>\</a></li>').show()

            }
        }
        $('.pagination li:first-child').addClass('active')
        $('.pagination li').on('click',function (){
            var pageNum = $(this).attr('data-page')
            var trIndex = 0;
            $('.pagination li').removeClass('active')
            $(this).addClass('active')
            $(table +' tr:gt(0)' ).each(function (){
                trIndex++
                if (trIndex >(marRows*pageNum)|| trIndex <= ((marRows*pageNum)-marRows)){
                    $(this).hide()
                }else {
                    $(this).show()
                }

            })
        })
    })

}
pageAcount();
$(function (){
    // $('table tr:eq(0)').prepend('<th>#</th>')
    var id =0;
    $('table tr:gt(0)').each(function (){
        id++
        $(this).prepend('<td>' +id+'</td>')
    })
})

page.initializeControlEvent = () => {
    page.loadData.getAllUser()
}

$(() => {
    page.initializeControlEvent()
})